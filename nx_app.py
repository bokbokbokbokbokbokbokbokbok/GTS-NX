...
if 'bldg_data' not in st.session_state:
    st.session_state['bldg_data'] = None
if 'buffer_poly_wgs84' not in st.session_state:
    st.session_state['buffer_poly_wgs84'] = None
if 'multi_line_wgs84' not in st.session_state:
    st.session_state['multi_line_wgs84'] = None
...
with tab1:
    ...
    with col_s2:
        ...
        if st.session_state['bldg_data'] is not None and len(st.session_state['bldg_data']) > 0:
            ...
            # [UI 배치 수정] 표보다 상단에 퀵 줌인 컨트롤러를 배치하여 직관적으로 선택 가능
            st.markdown("### 🎯 대상 건축물 퀵 줌인 컨트롤러")
            st.info("💡 아래 셀렉트박스에서 **연번과 명칭**을 선택하시면 즉시 해당 건물 위치로 지도가 확대(줌인)됩니다.")
